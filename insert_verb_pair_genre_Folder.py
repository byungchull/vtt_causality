...
for genreFolder in next(os.walk(path))[1]:
    for files in next(os.walk(path+genreFolder))[2]:
        ...
        print genreFolder, " - ", file_cnt, ": " ,files
        ...
        print ("File Read Time : %s seconds"%read_time)
        ...
        for i in verbDic:
            #print i, verbDic[i]
            try :
                con.execute("INSERT INTO pmiScoreGenre(eventPair, count, genre) VALUES(?, ?, ?)", (i, verbDic[i], genreFolder,))
            except sqlite3.Error as e :
                #print 'error :', e.message
                con.execute("UPDATE pmiScoreGenre SET count = count + ? WHERE genre=? AND eventPair=?", (verbDic[i], genreFolder, i,))

        con.close()

        end_time = (time.time() - start_time)
        print ("Database Process Time : %s seconds" % (time.time() - read_time))
        print ("Exec Ending Time : %s seconds"%end_time)
