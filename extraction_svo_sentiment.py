...
#genreList = ['Action','Adventure','Animation','Biography','tech','']
for genreFolder in next(os.walk(path))[1]:
    #if (genreFolder not in genreList):
    if (genreFolder in ("")):
        continue

    for files in next(os.walk(path+genreFolder))[2]:
        print genreFolder+"/"+files
        #f = open(path + genreFolder + "/" + files, "r")

        fileSeq = fileSeq + 1
        con.execute("INSERT INTO fileTBL (seq, genre, fileName, corpus) VALUES(?, ?, ?, ?)", (fileSeq, genreFolder, files, 'Film', ))
        ...
        for strSentence in sentList:
            ...
            con.execute(
                "INSERT INTO sentenceTBL (fileSeq, seq, sentence, pos, neg, neu, com) VALUES(?, ?, ?, ?, ?, ?, ?)",
                (fileSeq, sentenceSeq, strSentence, sentiment["pos"], sentiment["neg"], sentiment["neu"], sentiment["compound"]),)
            ...
            if (None in sentence.relations["VP"] or None in sentence.relations["SBJ"] or None in sentence.relations[
                "OBJ"]):
                listVP.append("")
                listOBJ.append("")
                listSBJ.append("")
            ...
            if (maxID > 0) :
                for i in range(maxID):
                    ...
                    con.execute(
                            "INSERT INTO ChunkTBL (fileSeq, sentSeq, id, sbj, vp, obj) VALUES(?, ?, ?, ?, ?, ?)",
                            (fileSeq, sentenceSeq, i, subject, verbs, objects), )
